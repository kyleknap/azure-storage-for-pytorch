...
def _add_result_dir_to_tables(result_dir, tables):
    ...
    tables[metadata['write-method']][metadata['block-size']][metadata['model-size']] = f"{mean:.3f} ± {stddev:.3f}"
...
def _display_tables(tables, num_runs):
    ...
    for read_method, table in tables.items():
        if read_method == 'torch-save':
            read_method = 'torch.save()'
        if read_method == 'writeall':
            read_method = 'write()'
...
def _display_table(table):
    tabulate_table = []
    headers = None
    for block_size, model_sizes in table.items():
        if headers is None:
            headers = ['Block size', *model_sizes.keys()]
        tabulate_table.append([block_size, *model_sizes.values()])
